...
@property (nonatomic, strong) CALayer * viewColor;
...
- (void)layoutSubviews {
    ...
    if (viewColor != nil) {
        [viewColor removeFromSuperlayer];
    }
    viewColor = [CALayer layer];
    viewColor.frame = CGRectMake(self.leftHandle.position.x, self.sliderLine.frame.origin.y, self.rightHandle.position.x - self.leftHandle.position.x, 5);
    viewColor.backgroundColor = [UIColor redColor].CGColor;
    [self.layer insertSublayer:viewColor above:self.sliderLine];
