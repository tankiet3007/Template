...
@property (nonatomic, strong) NSTimer *timeSliding;
...
- (void)setSlideItemWithDelay:(BOOL)slideItemWithDelay {
    _slideItemWithDelay = slideItemWithDelay;
    if(slideItemWithDelay == YES) {
        [self stopOrStartSliding:NO];
    }
}
...
- (void)stopOrStartSliding:(BOOL)isStop {
    if (isStop) {
        if (_timeSliding) {
            [_timeSliding invalidate];
            _timeSliding = nil;
        }
    }
    else {
        if (_timeSliding == nil) {
            self.timeSliding = [NSTimer scheduledTimerWithTimeInterval:3 target:self selector:@selector(scrollingTimer) userInfo:nil repeats:YES];
        }
    }
}

- (void)scrollingTimer {
    int next = (self.currentItemIndex + 1) > _numberOfItems?0:(self.currentItemIndex + 1);
    [self scrollToItemAtIndex:next animated:YES];
}
...
- (void)startDecelerating
{
    ...
    {
        _decelerating = YES;
//        [self performSelector:@selector(startAnimation) withObject:self afterDelay:3.0];
        [self startAnimation];
    }
}
