...
- (id)initWithFrame:(CGRect)frame andRating:(int)rating withLabel:(BOOL)label animated:(BOOL)animated
{
    ...
    if (self) {
        ...
        if (label) {
            ...
            self.label = [[[UILabel alloc]initWithFrame:CGRectMake(self.bounds.size.width-kLabelAllowance , 0,kLabelAllowance, frame.size.height)] autorelease];
            
            self.label.font = kSystemFonSizeRevert(10);
        ...
        starBackground.contents = (__bridge id)([UIImage imageNamed:@"start_grey.png"].CGImage);
        ...
        starMask.contents = (__bridge id)([UIImage imageNamed:@"start_grey.png"].CGImage);
...
- (void)setTitle:(NSString *)title withRate:(float)value {
    self.label.text = title;
    self.userRating = (value+1)*20.0f;
